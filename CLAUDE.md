...
## Система теней и глубины

### CSS-токены (определены в `style.css :root {}`)

Все тени и отступы используются через CSS-переменные. **Не хардкодить значения напрямую.**

#### Тени — состояние покоя (многоуровневые)

| Переменная | Применение |
|---|---|
| `--shadow-sm` | Breadcrumbs, подсказки, мелкие UI-элементы |
| `--shadow-md` | Стандартные карточки в состоянии покоя |
| `--shadow-lg` | Крупные блоки (search-card, profile-hero) |
| `--shadow-xl` | Hero-карточки поверх тёмного фона (tournament-description-card) |

#### Тени — hover (teal-свечение)

| Переменная | Применение |
|---|---|
| `--shadow-hover` | Стандартный hover на большинстве карточек |
| `--shadow-hover-deep` | Hover на акцентных/важных карточках (section-card, category-card) |
| `--shadow-hover-lg` | Hover на hero-уровне (search-cta-card) |

#### Правила теней

1. Все карточки в покое — минимум `--shadow-md` (не `box-shadow: none`)
2. Hover-тени всегда teal-тонированные через `--shadow-hover*` переменные
3. Тени многоуровневые: первый слой — мягкий, второй — глубокий teal-glow

### Подъём при hover (translateY)

- Мелкие интерактивные элементы (кнопки, теги, timeline-cards): `-2px`
- Стандартные карточки (stat-card, feature-card, type-card): `-6px`
- Акцентные вертикальные карточки (category-card): `-5px`
- Строки таблиц: не поднимаются, используют `translateX(4px)` для акцента

### Токены отступов

| Переменная | Значение | Применение |
|---|---|---|
| `--section-padding` | `40px 0` | Большинство секций страниц |
| `--section-padding-sm` | `30px 0` | Hero-блоки, страницы с формами |
| `--card-padding` | `22px 24px` | Основные карточки |
| `--card-padding-sm` | `16px` | Компактные карточки (tournament, inline) |
| `--grid-gap` | `16px` | Основные сетки карточек |
| `--grid-gap-sm` | `10px` | Плотные сетки (категории, фильтры) |

**Правило:** Новые секции и карточки должны использовать эти токены — не хардкодить `20px`, `30px`, `50px` напрямую.
...
- **Пишутся от первого лица, в прошедшем времени** — как будто разработчик описывает, что он сделал
- **Язык — русский**
- **Не упоминать AI, Claude или автогенерацию** — коммиты пишутся от лица разработчика

**Примеры:**
```
feat: добавил страницу профиля дзюдоиста с медалями
fix: исправил форматирование даты турнира
refactor: вынес компонент поиска в отдельный файл
docs: обновил README с инструкциями по Docker
chore: добавил .gitignore для Go проекта
```